-module(cryptoapis_list_transactions_by_address_ribs).

-export([encode/1]).

-export_type([cryptoapis_list_transactions_by_address_ribs/0]).

-type cryptoapis_list_transactions_by_address_ribs() ::
    #{ 'locktime' := integer(),
       'size' := integer(),
       'vSize' := integer(),
       'version' := integer(),
